...
public class PluginDefaultService {
    ...
    @Inject
    protected java.util.Optional<NamespacePluginDefaultsRepositoryInterface> namespacePluginDefaultsRepository;
    ...
    protected List<PluginDefault> getAllDefaults(final String tenantId,
        ...
        defaults.addAll(getNamespaceDefaults(tenantId, namespace));
        defaults.addAll(getGlobalDefaults());
        return defaults;
    }

    /**
     * Gets the namespace-level plugin defaults (when repository is available).
     */
    protected List<PluginDefault> getNamespaceDefaults(String tenantId, String namespace) {
        if (namespacePluginDefaultsRepository.isEmpty()) {
            return List.of();
        }
        return namespacePluginDefaultsRepository.get().getDefaultsByNamespace(tenantId, namespace);
    }
