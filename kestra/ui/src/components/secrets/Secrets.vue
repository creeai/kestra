<template>
    ...
    <section class="d-flex flex-column fill-height padding-bottom container">
        <div v-if="showOssPromoBlock" class="d-flex flex-column text-start m-0 p-0 mw-100">
        ...
        <SecretsTable
            v-else-if="showOssSecretsEnabled"
            filterable
            :namespace="miscStore.configs?.systemNamespace ?? 'system'"
            :addSecretModalVisible="addSecretModalVisible"
            @update:add-secret-modal-visible="addSecretModalVisible = $event"
...
<script setup lang="ts">
    ...
    const showOssPromoBlock = computed(
        () => miscStore.configs?.secretsEnabled === undefined
    );
    const showOssSecretsEnabled = computed(
        () => miscStore.configs?.secretsEnabled === true && (miscStore.configs?.edition === "OSS" || props.namespace === undefined)
    );
